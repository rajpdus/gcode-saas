...
def call_edit_file_tool(target_file: str, code_edit: str, instructions: str):
    ...
    try:
        # 'w' mode will create the file if it doesn't exist, or truncate (overwrite) it if it does.
        with open(target_file, 'w') as f:
            f.write(code_edit)
        print(f"  Successfully wrote/overwrote file: {target_file}")
        return {"status": "success"}
    except Exception as e:
        print(f"  Error writing/overwriting file {target_file}: {e}", file=sys.stderr)
        return {"status": "error", "message": f"Failed to write/overwrite file: {str(e)}"}
...
def parse_plan_with_llm(client: GeminiClient, plan_text: str, verbose: bool = False) -> List[Dict]:
    ...
    """
    parsing_prompt = f"""Your sole output MUST be a single, valid JSON list of objects. Do not include any other text, explanations, or markdown formatting outside of this JSON list.

Analyze the following plan text generated by an AI agent. Extract all file creation or modification instructions.
Each object in the JSON list must represent one file action and have the following keys:

- "action": A string. This field is mandatory. The value must be *only* "create" or "modify".
- "path": A string representing the relative file path (e.g., "src/app.py" or "docs/README.md"). This field is mandatory.
- "content": A string containing the full intended textual content of the file. This can be multi-line. For code, include all necessary imports, function/class structures, and comments as described in the plan. This field is mandatory.
- "is_complete": A boolean (true or false) indicating if the 'content' represents a complete and final implementation (true) or if it's a skeleton, placeholder, or requires further expansion (false). This field is mandatory.
- "priority": An integer from 1 (highest priority - essential structure files) to 3 (lowest priority - optional/enhancement files). This field is mandatory.
- "file_type": A string, typically "code" or "documentation", indicating the nature of the file. This field is mandatory.

Ensure that the `content` field accurately captures all necessary code, including imports, class/function definitions, and relevant comments as described in the plan.
For large or complex files, the 'content' may represent placeholder or skeleton code; in such cases, 'is_complete' should be set to false.
...
""" if not is_initial_plan else ""
    ...
    flake8_checked_once = False # Flag to ensure we only print "flake8 not found" once
    flake8_path = shutil.which('flake8') # Check for flake8 once
        ...
        if action == "create":
                    ...
                    try:
                        ...
                        if result.get("status") == "success":
                            ...
                            if target_file_path.endswith(".py"):
                                if flake8_path:
                                    try:
                                        process = subprocess.run([flake8_path, target_file_path], capture_output=True, text=True, check=False)
                                        if process.returncode != 0:
                                            status_note += f" (Linted with flake8 - issues found):\n{process.stdout.strip()}"
                                        else:
                                            status_note += " (Linted with flake8 - no issues)"
                                    except Exception as e:
                                        status_note += f" (Flake8 linting failed: {e})"
                                else:
                                    if not flake8_checked_once:
                                        print("\nWarning: flake8 not found. Python linting will be skipped. Please install flake8.")
                                        flake8_checked_once = True
                                    status_note += " (Not linted - flake8 not found)"
            ...
            if auto_apply:
                if os.path.exists(target_file_path):
                    print(f"Attempting to MODIFY file: '{relative_path}' in outputs directory")
                    try:
                        instructions = f"Modify the existing file '{relative_path}' in outputs directory as specified in the plan for step {step_arg}."
                        result = call_edit_file_tool(
                            target_file=target_file_path,
                            code_edit=content,
                            instructions=instructions
                        ...
                        if result.get("status") == "success":
                            print(f"Successfully applied modification for: {relative_path}")
                            status_note = " (Modified in outputs directory - Partial Update)" if not is_complete else " (Modified in outputs directory)"
                            if target_file_path.endswith(".py"):
                                if flake8_path:
                                    try:
                                        process = subprocess.run([flake8_path, target_file_path], capture_output=True, text=True, check=False)
                                    ...
                                    # No need to print general warning again if already done for create
                                    if not flake8_checked_once:
                                        print("\nWarning: flake8 not found. Python linting will be skipped. Please install flake8.")
                                        flake8_checked_once = True
                                    status_note += " (Not linted - flake8 not found)"
                            ...
                            print(f"Error modifying file: {result.get('message', 'Unknown error')}")
                            execution_success = False
                            needs_manual_review.append(relative_path + f" (Modification Failed - {result.get('message', 'Unknown error')})")
                    except Exception as tool_error:
                        print(f"Error applying tool to modify {relative_path}: {tool_error}", file=sys.stderr)
                        execution_success = False
                        needs_manual_review.append(relative_path + " (Tool Error - Modify)")
                else:
                    # File doesn't exist, but action is modify. Treat as create for auto-apply.
                    print(f"File '{relative_path}' does not exist for modification. Attempting to CREATE instead in outputs directory.")
                    try:
                        instructions = f"Create the file '{relative_path}' (originally planned as modify but file not found) in outputs directory for step {step_arg}."
                        result = call_edit_file_tool(
                            target_file=target_file_path,
                            code_edit=content,
                            instructions=instructions
                        ...
                        if result.get("status") == "success":
                            print(f"Successfully created (originally modify) file: {relative_path}")
                            status_note = " (Created in outputs directory - Skeleton/Placeholder)" if not is_complete else " (Created in outputs directory)"
                            if target_file_path.endswith(".py"):
                                if flake8_path:
                                    try:
                            ...
                            print(f"Error creating (originally modify) file: {result.get('message', 'Unknown error')}")
                            execution_success = False
                            needs_manual_review.append(relative_path + f" (Creation Failed for modify action - {result.get('message', 'Unknown error')})")
                    except Exception as tool_error:
                        print(f"Error applying tool to create (originally modify) {relative_path}: {tool_error}", file=sys.stderr)
                        execution_success = False
                        needs_manual_review.append(relative_path + " (Tool Error - Create for modify)")
            else: # auto_apply is False
                if os.path.exists(target_file_path):
                    ...
                    # Planned modification, but file doesn't exist, and no auto-apply. Flag for review.
